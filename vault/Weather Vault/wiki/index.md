...
- [[2026-04-22 Consensus-Fade v2 local-time anchoring]] — Consensus-Fade +1 rewritten: "20 UTC fixed" → "≥16:00 city-local". Same trade, cleaner coordinate system. Fixed v1 price-source bug that inflated headline. Canonical n=78/98.7%/OOS t=+4.49. 0.22-cap overlay (100% hit, t=+7.70) documented as optional tightening, NOT canonical.
- [[2026-04-14 Strategy D does NOT replicate in clean temporal holdout]] — ❌ **RETRACTS Strategy D V1/V2**. Pre-registered IS/OOS temporal holdout (IS Mar 11-31, OOS Apr 1-10) across 11 cities shows no pre-registered directional strategy survives. Exploratory market-fav−1 decayed +$0.058 IS → +$0.001 OOS (100% edge decay). Prior +$1.94/trade headline was period+city-specific artifact; market is approximately calibrated on 2°F buckets.
- [[2026-04-11 Near-resolution ladder-bid arbitrage]] — ❌ RETRACTED (expT slippage-aware check). Prior "verified edge" was an artifact of zero-fee + 1-second-blip price assumptions. Real break-even needs sum_bid ≥ 1.046 which only 1-2 of 90 observed windows reached. Realistic taker-arb capacity ≈ $0/day. Page retained as historical record.
- [[2026-04-11 Polymarket arb taker bot fingerprint]] — clustered last_trade_price events reveal a disciplined 5-share / 10-leg / 10-50ms SELL ladder-arb bot running ~2.5 clusters/hour on NYC. ⚠️ Post-fee-aware-correction: some observed "arbs" were actually directional trades. See fee structure synthesis.
- [[2026-04-11 Polymarket fee structure + maker rebate pivot]] — ⚠️ MAJOR correction. Weather fees = C × 0.05 × p × (1-p). Break-even sum_bid for 5sh 11-bucket arb is 1.036 (only ~5% of windows cross). Taker arb capacity revised 10x DOWN to $1-8/day NYC. Maker rebate path (25% of fees) is 10-20x more lucrative at $60-150/day — strategy pivot recommended.
...
- [[Polymarket CLOB execution]] — REST + WSS order placement via `py-clob-client`: auth derivation, allowances, order types, cancellation, rate limits. Deployment reference for `strategies/consensus_fade_plus1/`.
